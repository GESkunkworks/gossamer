...
// ConvertLegacyFlagsToConfig takes a GossFlags struct that's built using command parameters
// via the flag package and converts them into the Config struct that gossamer will use for
// the majority of its operations. Mainly used to help backward compatibility with gossamer 1.x
func (gc *Config) ConvertLegacyFlagsToConfig(gfl *GossFlags) (err error) {
	...
	if gfl.RoleArn == "" && gfl.RolesFile == "" {
		err = errors.New("legacy: must specify role ARN with '-a' or '-rolesfile'")
		return err
	}
	if gfl.RoleArn != "" && gfl.RolesFile == "" {
	...
	if len(mappings) == 0 {
		err = errors.New("must specify role ARN with '-a' or '-rolesfile'")
		return err
	}
	if gfl.SessionDuration == 0 {
		if 900 > gfl.SessionDuration {
			err = errors.New("sessionDuration is outside threshold min=900")
			return err
		}
		flow.DurationSeconds = gfl.SessionDuration
	}
